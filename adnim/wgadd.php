...
$wg_add = NULL; // Initialize page object first

class cwg_add extends cwg {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["id_wg"] != "") {
				...
				$this->setKey("id_wg", $this->id_wg->CurrentValue); // Set up key
			} else {
				$this->setKey("id_wg", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "wgview.php")
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			$this->id_wg->setDbValue($conn->Insert_ID());
			$rsnew['id_wg'] = $this->id_wg->DbValue;
		}
		if ($AddRow) {
...
// Create page object
if (!isset($wg_add)) $wg_add = new cwg_add();
...
// Page object
var wg_add = new ew_Page("wg_add");
wg_add.PageID = "add"; // Page ID
var EW_PAGE_ID = wg_add.PageID; // For backward compatibility

// Form object
var fwgadd = new ew_Form("fwgadd");

// Validate form
fwgadd.Validate = function(fobj) {
...
fwgadd.ValidateRequired = true;
<?php } else { ?>
fwgadd.ValidateRequired = false; 
<?php } ?>
...
<p><span id="ewPageCaption" class="ewTitle ewTableTitle"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $wg->TableCaption() ?></span></p>
<p class="phpmaker"><a href="<?php echo $wg->getReturnUrl() ?>" id="a_GoBack" class="ewLink"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $wg_add->ShowPageHeader(); ?>
<?php
$wg_add->ShowMessage();
?>
<form name="fwgadd" id="fwgadd" class="ewForm" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return ewForms[this.id].Submit();">
<br>
<input type="hidden" name="t" value="wg">
<input type="hidden" name="a_add" id="a_add" value="A">
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_wgadd" class="ewTable">
