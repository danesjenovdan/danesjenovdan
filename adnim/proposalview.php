...
$proposal_view = NULL; // Initialize page object first

class cproposal_view extends cproposal {
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["id_proposal"] <> "") {
			$this->RecKey["id_proposal"] = $_GET["id_proposal"];
			$KeyUrl .= "&id_proposal=" . urlencode($this->RecKey["id_proposal"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id_proposal"] <> "") {
				...
				$this->RecKey["id_proposal"] = $this->id_proposal->QueryStringValue;
			} else {
				$sReturnUrl = "proposallist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "proposallist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "proposallist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($proposal_view)) $proposal_view = new cproposal_view();

// Page init
$proposal_view->Page_Init();

// Page main
$proposal_view->Page_Main();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var proposal_view = new ew_Page("proposal_view");
proposal_view.PageID = "view"; // Page ID
var EW_PAGE_ID = proposal_view.PageID; // For backward compatibility

// Form object
var fproposalview = new ew_Form("fproposalview");

// Form_CustomValidate event
fproposalview.Form_CustomValidate =  
...
fproposalview.ValidateRequired = true;
<?php } else { ?>
fproposalview.ValidateRequired = false; 
<?php } ?>
...
<p><span id="ewPageCaption" class="ewTitle ewTableTitle"><?php echo $Language->Phrase("View") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $proposal->TableCaption() ?>&nbsp;&nbsp;</span><?php $proposal_view->ExportOptions->Render("body"); ?>
</p>
<p class="phpmaker">
<a href="<?php echo $proposal_view->ListUrl ?>" id="a_BackToList" class="ewLink"><?php echo $Language->Phrase("BackToList") ?></a>&nbsp;
<?php if ($proposal_view->AddUrl <> "") { ?>
<a href="<?php echo $proposal_view->AddUrl ?>" id="a_AddLink" class="ewLink"><?php echo $Language->Phrase("ViewPageAddLink") ?></a>&nbsp;
<?php } ?>
<?php if ($proposal_view->EditUrl <> "") { ?>
<a href="<?php echo $proposal_view->EditUrl ?>" id="a_EditLink" class="ewLink"><?php echo $Language->Phrase("ViewPageEditLink") ?></a>&nbsp;
<?php } ?>
<?php if ($proposal_view->CopyUrl <> "") { ?>
<a href="<?php echo $proposal_view->CopyUrl ?>" id="a_CopyLink" class="ewLink"><?php echo $Language->Phrase("ViewPageCopyLink") ?></a>&nbsp;
<?php } ?>
<?php if ($proposal_view->DeleteUrl <> "") { ?>
<a href="<?php echo $proposal_view->DeleteUrl ?>" id="a_DeleteLink" class="ewLink"><?php echo $Language->Phrase("ViewPageDeleteLink") ?></a>&nbsp;
<?php } ?>
</p>
<?php $proposal_view->ShowPageHeader(); ?>
<?php
$proposal_view->ShowMessage();
?>
<form name="fproposalview" id="fproposalview" class="ewForm" action="" method="post">
<input type="hidden" name="t" value="proposal">
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_proposalview" class="ewTable">
...
<?php echo $proposal->timestamp->ViewValue ?></span>
</span></td>
	</tr>
<?php } ?>
</table>
