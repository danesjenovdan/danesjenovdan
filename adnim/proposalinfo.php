...
//
// Table class for proposal
//
class cproposal extends cTable {
	var $id_proposal;
	var $id_right;
	var $id_user;
	var $title;
	...
	function __construct() {
		...
		// id_proposal
		$this->id_proposal = new cField('proposal', 'proposal', 'x_id_proposal', 'id_proposal', '`id_proposal`', '`id_proposal`', 3, -1, FALSE, '`id_proposal`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id_proposal->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_proposal'] = &$this->id_proposal;

		// id_right
		$this->id_right = new cField('proposal', 'proposal', 'x_id_right', 'id_right', '`id_right`', '`id_right`', 3, -1, FALSE, '`id_right`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id_right->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_right'] = &$this->id_right;

		// id_user
		$this->id_user = new cField('proposal', 'proposal', 'x_id_user', 'id_user', '`id_user`', '`id_user`', 3, -1, FALSE, '`id_user`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id_user->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_user'] = &$this->id_user;

		// title
		$this->title = new cField('proposal', 'proposal', 'x_title', 'title', '`title`', '`title`', 200, -1, FALSE, '`title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['title'] = &$this->title;

		// text
		$this->text = new cField('proposal', 'proposal', 'x_text', 'text', '`text`', '`text`', 201, -1, FALSE, '`text`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['text'] = &$this->text;

		// approved
		$this->approved = new cField('proposal', 'proposal', 'x_approved', 'approved', '`approved`', '`approved`', 3, -1, FALSE, '`approved`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->approved->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['approved'] = &$this->approved;

		// timestamp
		$this->timestamp = new cField('proposal', 'proposal', 'x_timestamp', 'timestamp', '`timestamp`', 'DATE_FORMAT(`timestamp`, \'%Y/%m/%d %H:%i:%s\')', 135, 5, FALSE, '`timestamp`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			$sql .= ew_QuotedName('id_proposal') . '=' . ew_QuotedValue($rs['id_proposal'], $this->id_proposal->FldDataType) . ' AND ';
		}
	...
	function SqlKeyFilter() {
		return "`id_proposal` = @id_proposal@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->id_proposal->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@id_proposal@", ew_AdjustSql($this->id_proposal->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	// View URL
	function GetViewUrl() {
		return $this->KeyUrl("proposalview.php", $this->UrlParm());
	}

	// Add URL
	function GetAddUrl() {
		return "proposaladd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("proposaledit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("proposaladd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("proposaldelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->id_proposal->CurrentValue)) {
			$sUrl .= "id_proposal=" . urlencode($this->id_proposal->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["id_proposal"]; // id_proposal
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->id_proposal->CurrentValue = $key;
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->id_right->Exportable) $Doc->ExportCaption($this->id_right);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->id_right->Exportable) $Doc->ExportField($this->id_right);
