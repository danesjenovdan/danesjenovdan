...
//
// Table class for vote
//
class cvote extends cTable {
	var $id_vote;
	var $id_argument;
	var $id_proposal;
	var $id_user;
	...
	function __construct() {
		...
		// id_vote
		$this->id_vote = new cField('vote', 'vote', 'x_id_vote', 'id_vote', '`id_vote`', '`id_vote`', 3, -1, FALSE, '`id_vote`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id_vote->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_vote'] = &$this->id_vote;

		// id_argument
		$this->id_argument = new cField('vote', 'vote', 'x_id_argument', 'id_argument', '`id_argument`', '`id_argument`', 3, -1, FALSE, '`id_argument`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id_argument->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_argument'] = &$this->id_argument;

		// id_proposal
		$this->id_proposal = new cField('vote', 'vote', 'x_id_proposal', 'id_proposal', '`id_proposal`', '`id_proposal`', 3, -1, FALSE, '`id_proposal`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id_proposal->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_proposal'] = &$this->id_proposal;

		// id_user
		$this->id_user = new cField('vote', 'vote', 'x_id_user', 'id_user', '`id_user`', '`id_user`', 3, -1, FALSE, '`id_user`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id_user->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_user'] = &$this->id_user;

		// type
		$this->type = new cField('vote', 'vote', 'x_type', 'type', '`type`', '`type`', 3, -1, FALSE, '`type`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->type->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['type'] = &$this->type;

		// vote_plus
		$this->vote_plus = new cField('vote', 'vote', 'x_vote_plus', 'vote_plus', '`vote_plus`', '`vote_plus`', 3, -1, FALSE, '`vote_plus`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->vote_plus->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['vote_plus'] = &$this->vote_plus;

		// vote_minus
		$this->vote_minus = new cField('vote', 'vote', 'x_vote_minus', 'vote_minus', '`vote_minus`', '`vote_minus`', 3, -1, FALSE, '`vote_minus`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->vote_minus->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['vote_minus'] = &$this->vote_minus;

		// timestamp
		$this->timestamp = new cField('vote', 'vote', 'x_timestamp', 'timestamp', '`timestamp`', 'DATE_FORMAT(`timestamp`, \'%Y/%m/%d %H:%i:%s\')', 135, 5, FALSE, '`timestamp`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			$sql .= ew_QuotedName('id_vote') . '=' . ew_QuotedValue($rs['id_vote'], $this->id_vote->FldDataType) . ' AND ';
		}
	...
	function SqlKeyFilter() {
		return "`id_vote` = @id_vote@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->id_vote->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@id_vote@", ew_AdjustSql($this->id_vote->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	// View URL
	function GetViewUrl() {
		return $this->KeyUrl("voteview.php", $this->UrlParm());
	}
	...
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("voteedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("voteadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("votedelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->id_vote->CurrentValue)) {
			$sUrl .= "id_vote=" . urlencode($this->id_vote->CurrentValue);
		} else {
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				if ($this->id_vote->Exportable) $Doc->ExportCaption($this->id_vote);
				...
				if ($this->vote_plus->Exportable) $Doc->ExportCaption($this->vote_plus);
				if ($this->vote_minus->Exportable) $Doc->ExportCaption($this->vote_minus);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					if ($this->id_vote->Exportable) $Doc->ExportField($this->id_vote);
					...
					if ($this->vote_plus->Exportable) $Doc->ExportField($this->vote_plus);
					if ($this->vote_minus->Exportable) $Doc->ExportField($this->vote_minus);
