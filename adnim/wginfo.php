...
class cwg extends cTable {
	...
	function __construct() {
		...
		// id_wg
		$this->id_wg = new cField('wg', 'wg', 'x_id_wg', 'id_wg', '`id_wg`', '`id_wg`', 3, -1, FALSE, '`id_wg`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id_wg->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_wg'] = &$this->id_wg;

		// id_user
		$this->id_user = new cField('wg', 'wg', 'x_id_user', 'id_user', '`id_user`', '`id_user`', 3, -1, FALSE, '`id_user`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id_user->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_user'] = &$this->id_user;

		// id_right
		$this->id_right = new cField('wg', 'wg', 'x_id_right', 'id_right', '`id_right`', '`id_right`', 3, -1, FALSE, '`id_right`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id_right->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_right'] = &$this->id_right;

		// id_proposal
		$this->id_proposal = new cField('wg', 'wg', 'x_id_proposal', 'id_proposal', '`id_proposal`', '`id_proposal`', 3, -1, FALSE, '`id_proposal`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id_proposal->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_proposal'] = &$this->id_proposal;

		// timestamp
		$this->timestamp = new cField('wg', 'wg', 'x_timestamp', 'timestamp', '`timestamp`', 'DATE_FORMAT(`timestamp`, \'%Y/%m/%d %H:%i:%s\')', 135, 5, FALSE, '`timestamp`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			$sql .= ew_QuotedName('id_wg') . '=' . ew_QuotedValue($rs['id_wg'], $this->id_wg->FldDataType) . ' AND ';
		}
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->id_wg->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@id_wg@", ew_AdjustSql($this->id_wg->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	// View URL
	function GetViewUrl() {
		return $this->KeyUrl("wgview.php", $this->UrlParm());
	}
	...
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("wgedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("wgadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("wgdelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->id_wg->CurrentValue)) {
			$sUrl .= "id_wg=" . urlencode($this->id_wg->CurrentValue);
		} else {
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				if ($this->id_wg->Exportable) $Doc->ExportCaption($this->id_wg);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					if ($this->id_wg->Exportable) $Doc->ExportField($this->id_wg);
